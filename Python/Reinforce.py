...
class ComebotEnv(gym.Env):  # gym.Env 상속
    ...
    def step(self, action):
        ...
        if recv_packet[0] == "ID:":
            ...
            self.Touch_Sensor = int(recv_packet[3][1])
            self.Force_Sensor = int(recv_packet[4][1])
            self.Lift_Sensor = int(recv_packet[5][1])
            self.Oled_State = int(recv_packet[6][1])
            self.Fin_State = int(recv_packet[7][1])
            self.Tail_State = int(recv_packet[8][1])
        ...
        print('obs:    ', obs, '       Face:', self.Face_Detect, '       reward:', reward)
